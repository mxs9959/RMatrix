...
#import sys
#sys.path.append('/home/mxs/RMatrix')
#sys.path.append('/home/mxs/rpi-rgb-led-matrix/bindings/python')
import matrix_tools
from rgbmatrix import graphics

s = '/home/mxs/rpi-rgb-led-matrix/fonts/'
...
def refresh():
	...
	response = requests.get('https://api.open-meteo.com/v1/forecast?latitude=33.749&longitude=-84.388&hourly=temperature_2m,relativehumidity_2m,apparent_temperature,precipitation_probability,precipitation,surface_pressure,cloudcover,visibility,windspeed_10m,winddirection_10m&daily=temperature_2m_max,temperature_2m_min,sunrise,sunset,uv_index_max,precipitation_hours&temperature_unit=fahrenheit&windspeed_unit=mph&precipitation_unit=inch&timezone=America%2FNew_York&past_days=1')
...
def stickScroll(strings, font, color1, color2, pd):
	fontX = graphics.Font()
	fontX.LoadFont(s + font)
	...
	while len(displaying) > 0:
		for desc in displaying:
			if desc[1]-pos == 1 and desc[0]%2 == 0:
				...
				length = graphics.DrawText(m, fontX, desc[1]-pos, Y, colors[desc[0]%2], strings[desc[0]])
				desc[2] = length
		...
		if stuck != None:
			...
			graphics.DrawText(m, fontX, 1, Y, colors[0], strings[stuck[0]])
			stuck = None
		sleep(pd)
		pos += 1
		m.Clear()
...
if __name__ == '__main__':
	load()
	hourly()
	stickScroll(ticker, '5x7.bdf', matrix_tools.randVisColor(100), matrix_tools.randVisColor(100), 0.05)
